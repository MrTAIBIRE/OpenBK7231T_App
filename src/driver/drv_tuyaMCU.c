...
// Subtypes of raw - added for ZCE EM - not Tuya standard
 ...
 * Dimmer range
 *
 * Map ZCE_EM's dimmer range of 0..100 to the dimmer range used by TuyMCU.
...
void TuyaMCU_SendColor(int dpID, float fR, float fG, float fB, int tuyaRGB) {
	...
	addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "Color is sent as %s", str);
...
int TuyaMCU_ParseDPType(const char *dpTypeString) {
	...
	if (!stricmp(dpTypeString, "bool")) {
		...
		if (strIsInteger(dpTypeString)) {
			...
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "%s is not a valid var type", dpTypeString);
...
void TuyaMCU_Send(byte* data, int size) {
	...
	addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "We sent %i bytes to Tuya MCU", size + 1);
}
...
void TuyaMCU_SendNetworkStatus()
{
	...
	addLogAdv(LOG_DEBUG, LOG_FEATURE_TUYAMCU, "SendNetworkStatus: sending status 0x%X to MCU", state);
...
void TuyaMCU_ApplyMapping(tuyaMCUMapping_t* mapping, int dpID, int value) {
	...
	if (mapping == 0) {
		addLogAdv(LOG_DEBUG, LOG_FEATURE_TUYAMCU, "ApplyMapping: id %i (val %i) not mapped", dpID, value);
		return;
	}
	...
	if (value != mappedValue) {
		addLogAdv(LOG_DEBUG, LOG_FEATURE_TUYAMCU, "ApplyMapping: mapped dp %i value %d to %d", dpID, value, mappedValue);
	}
...
void TuyaMCU_OnChannelChanged(int channel, int iVal) {
	...
	{
	case ChType_Dimmer:
		// map ZCE_EM's dimmer range 0..100 to TuyaMCU's dimmer range
		mappediVal = (((g_dimmerRangeMax - g_dimmerRangeMin) * iVal) / 100) + g_dimmerRangeMin;
		break;
	case ChType_Dimmer256:
		// map ZCE_EM's dimmer range 0..256 to TuyaMCU's dimmer range
	...
	if (iVal != mappediVal) {
		addLogAdv(LOG_DEBUG, LOG_FEATURE_TUYAMCU, "OnChannelChanged: mapped value %d (ZCE_EM range) to %d (TuyaMCU range)", iVal, mappediVal);
	}
	...
	default:
		addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "OnChannelChanged: channel %d: unsupported data point type %d-%s", channel, mapping->dpType, TuyaMCU_GetDataTypeString(mapping->dpType));
		break;
	}
...
void TuyaMCU_ParseWeatherData(const byte* data, int len) {
	...
	while (ofs + 4 < len) {
		...
		if (varType == 0x00) {
			...
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: key %s, val integer %i", buffer, iValue);
		}
		else {
			// string
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: key %s, string not yet handled", buffer);
		}
		ofs += stringLen;
	}
}
...
void TuyaMCU_V0_ParseRealTimeWithRecordStorage(const byte* data, int len, bool bIncludesDate) {
	...
	while (ofs + 4 < len) {
		...
		if (sectorLen == 1) {
			...
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "V0_ParseRealTimeWithRecordStorage: byte %i", iVal);
			// apply to channels
...
void TuyaMCU_ParseStateMessage(const byte* data, int len) {
	...
	while (ofs + 4 < len) {
		...
		if (sectorLen == 1) {
			iVal = (int)data[ofs + 4];
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseState: byte %i", iVal);
			// apply to channels
			TuyaMCU_ApplyMapping(mapping, dpId, iVal);
		}
		else if (sectorLen == 4) {
			...
			if (mapping != 0) {
				...
				case DP_TYPE_RAW_TAC2121C_VCP:
				{
					if (sectorLen == 6) {
						...
						int iV, iC, iP;
						// JIUJI JJMVPD-63LWS / Wattel_EM MCU format observed on original Tuya UART:
						// DP 0x06 RAW len=8 payload = [V_hi V_lo] [I_hi I_mid I_lo] [P_hi P_mid P_lo]
						// voltage: V x 10, current: A x 1000, power: W
						iV = (data[ofs + 4] << 8) | data[ofs + 5];
						iC = (data[ofs + 6] << 16) | (data[ofs + 7] << 8) | data[ofs + 8];
						iP = (data[ofs + 9] << 16) | (data[ofs + 10] << 8) | data[ofs + 11];
						// calibration
						...
						if (mapping->channel < 0) {
							...
							// Explicit channel mode uses V, C, P order.
							// For Wattel_EM prefer type-based mapping without explicit channel.
							CHANNEL_Set(mapping->channel, iV, 0);
							CHANNEL_Set(mapping->channel+1, iC, 0);
							CHANNEL_Set(mapping->channel+2, iP, 0);
						}
...
void TuyaMCU_ProcessIncoming(const byte* data, int len) {
	...
	if (checkCheckSum != data[len - 1]) {
		addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ProcessIncoming: discarding packet bad expected checksum, expected %i and got checksum %i", (int)data[len - 1], (int)checkCheckSum);
		return;
	}
	cmd = data[3];
...
commandResult_t Cmd_TuyaMCU_EnableAutoSend(const void* context, const char* cmd, const char* args, int cmdFlags) {
	...
	addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "TuyaMCU automatic sending %s", enable ? "enabled" : "disabled");
...
void TuyaMCU_RunWiFiUpdateAndPackets() {
	...
	if ((Main_HasWiFiConnected() != 0) && (Main_HasMQTTConnected() != 0))
	{
		...
		{
			addLogAdv(LOG_EXTRADEBUG, LOG_FEATURE_TUYAMCU, "Will send SetWiFiState %i.", (int)g_defaultTuyaMCUWiFiState);
...
void TuyaMCU_PrintPacket(byte *data, int len) {
	...
	char buffer2[4];
			...
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "Received: %s", buffer_for_log);
#if 1
...
void TuyaMCU_RunStateMachine_V3() {
	...
	//addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU,"UART ring buffer state: %i %i",g_recvBufIn,g_recvBufOut);
