...
void init_rest() {
	/* ZCE EM exposes REST only for local OTA upload. The full base WebApp is disabled. */
...
static int http_rest_get(http_request_t* request) {
	...
	return http_rest_error(request, 404, "ZCE EM API disabled");
}
...
#elif PLATFORM_W600 || PLATFORM_W800 || PLATFORM_BL602 || PLATFORM_BL_NEW || PLATFORM_LN882H || PLATFORM_LN8825 || PLATFORM_ESPIDF || PLATFORM_ESP8266 || PLATFORM_ECR6600 || PLATFORM_TR6260 || PLATFORM_RDA5981
	r = http_rest_post_flash(request, -1, -1);
#elif PLATFORM_REALTEK || PLATFORM_TXW81X || (PLATFORM_XRADIO && !PLATFORM_XR809)
	r = http_rest_post_flash(request, 0, -1);
#else
	ADDLOG_ERROR(LOG_FEATURE_API, "No OTA target configured");
#endif
	OTA_ResetProgress();
	return r;
}
...
static int http_rest_app(http_request_t* request)
{
	...
	http_html_start(request, "Application disabled");
	poststr(request, "ZCE EM exposes only Wi-Fi setup and OTA update.");
