...
void CSimulator::drawWindow() {
	...
	sprintf(buffer, "ZCE EM Simulator " __DATE__  " - %s", projectPathDisp);
	...
	float h = 40.0f;
	h = drawText(NULL, 10, h, "ZCE EM Simulator");
	if (sim != 0) {
...
void CSimulator::createWindow() {
	Window = SDL_CreateWindow("ZCE EM Simulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WinWidth, WinHeight, WindowFlags);
	assert(Window);
