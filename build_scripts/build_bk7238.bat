...
:: ========================================================
:: ZCE EM BK7238 Windows build script
:: Uses Git Bash + bundled ARM GCC toolchain from BK7231N
:: ========================================================

set APP_NAME=ZCE_EM7238_App
set APP_VERSION=dev_bk7238
...
echo ==============================================
echo Building ZCE EM for BK7238 on Windows
...
"%GIT_BASH%" --login -i "%CD%\build_scripts\build_bk72xx_freertos_inner.sh" "%APP_NAME%" "%APP_VERSION%" "%SDK_DIR%" "%OBK_VARIANT%" "%ACTION%" "bk7238"
set BUILD_RESULT=%errorlevel%

if %BUILD_RESULT% neq 0 (
    echo.
    echo [ERROR] BK7238 build failed with exit code %BUILD_RESULT%!
    exit /b %BUILD_RESULT%
)
...
if exist "%GEN_DIR%\bk7238_QIO.bin" (
    copy /y "%GEN_DIR%\bk7238_QIO.bin" "output\%APP_VERSION%\%APP_NAME%_QIO_%APP_VERSION%.bin" >nul 2>nul
