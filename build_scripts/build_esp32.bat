...
echo ==============================================
echo Building ZCE EM ESP32 on Windows natively
echo Target: %TARGET%
echo Size: %ESP_FSIZE%
...
python -m esptool -c %TARGET% merge_bin -o !FACTORY_BIN! --flash_mode dio --flash_size %ESP_FSIZE% %BOOTLOADER_ADDR% %BUILD_DIR%\bootloader\bootloader.bin 0x8000 %BUILD_DIR%\partition_table\partition-table.bin 0x10000 %BUILD_DIR%\ZCE EM.bin
if !errorlevel! neq 0 (
