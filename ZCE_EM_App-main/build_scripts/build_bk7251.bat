...
:: ========================================================
:: ZCE EM BK7251 Windows build script
:: Uses Git Bash + bundled ARM GCC toolchain from BK7231N
:: ========================================================

set APP_NAME=ZCE_EM7251_App
set APP_VERSION=dev_bk7251
...
echo ==============================================
echo Building ZCE EM for BK7251 on Windows
...
"%GIT_BASH%" --login -i "%CD%\build_scripts\build_bk72xx_freertos_inner.sh" "%APP_NAME%" "%APP_VERSION%" "%SDK_DIR%" "%OBK_VARIANT%" "%ACTION%" "bk7251"
set BUILD_RESULT=%errorlevel%

if %BUILD_RESULT% neq 0 (
    echo.
    echo [ERROR] BK7251 build failed with exit code %BUILD_RESULT%!
    exit /b %BUILD_RESULT%
)
...
if exist "%GEN_DIR%\bk7251_QIO.bin" (
    copy /y "%GEN_DIR%\bk7251_QIO.bin" "output\%APP_VERSION%\%APP_NAME%_QIO_%APP_VERSION%.bin" >nul 2>nul
    echo [INFO] Copied: %APP_NAME%_QIO_%APP_VERSION%.bin
    set /a COPIED_COUNT+=1
)
if exist "%GEN_DIR%\bk7251_UA.bin" (
    copy /y "%GEN_DIR%\bk7251_UA.bin" "output\%APP_VERSION%\%APP_NAME%_UA_%APP_VERSION%.bin" >nul 2>nul
