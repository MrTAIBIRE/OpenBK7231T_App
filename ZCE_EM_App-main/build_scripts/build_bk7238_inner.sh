...
echo "[INFO] Inner build script started for BK7238"

SCRIPT_DIR="$(pwd)"

# Export the borrowed ARM GCC toolchain to the environment for Beken FreeRTOS SDK Makefile to pick up
TOOLCHAIN_RAW="$(pwd)/sdk/ZCE_EM7231N/platforms/BK7231N/toolchain/windows/gcc-arm-none-eabi-4_9-2015q1/bin/"
...
if [ "$ACTION" = "clean" ]; then
    echo "[INFO] Running BK7238 clean from $(pwd)..."
    make clean -C ./
    exit 0
fi

echo "[INFO] Running BK7238 build from $(pwd)..."
...
# Replace encrypt.exe with the compatible one from ZCE_EM7231T SDK that supports multi-key arguments
cp "$(cygpath -u "$SCRIPT_DIR/sdk/ZCE_EM7231T/platforms/bk7231t/bk7231t_os/tools/generate/package_tool/windows/encrypt.exe")" ./tools/crc_binary/encrypt.exe

make TEMP="$LOCAL_TMP" TMP="$LOCAL_TMP" TMPDIR="$LOCAL_TMP" bk7238 -j8 USER_SW_VER="$APP_VERSION" OBK_DIR="../../src"
