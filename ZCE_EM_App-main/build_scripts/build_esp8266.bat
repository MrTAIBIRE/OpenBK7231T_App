...
echo ==============================================
echo Building ZCE EM ESP8266 on Windows natively
echo Size: %ESP_FSIZE%
...
python -m esptool --chip esp8266 merge_bin -o !FACTORY_BIN! --flash_mode dout --flash_size %ESP_FSIZE% %BOOTLOADER_ADDR% %BUILD_DIR%\bootloader\bootloader.bin 0x8000 %BUILD_DIR%\partition_table\partition-table.bin 0x10000 %BUILD_DIR%\ZCE EM.bin
if !errorlevel! neq 0 (
    :: Try with esptool.py directly from SDK
    echo [INFO] Retrying with SDK esptool...
    python "%IDF_PATH%\components\esptool_py\esptool\esptool.py" --chip esp8266 merge_bin -o !FACTORY_BIN! --flash_mode dout --flash_size %ESP_FSIZE% %BOOTLOADER_ADDR% %BUILD_DIR%\bootloader\bootloader.bin 0x8000 %BUILD_DIR%\partition_table\partition-table.bin 0x10000 %BUILD_DIR%\ZCE EM.bin
    if !errorlevel! neq 0 (
...
copy /y %BUILD_DIR%\ZCE EM.bin !IMG_BIN! >nul
