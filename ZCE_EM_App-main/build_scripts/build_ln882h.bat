...
echo ==============================================
echo Building ZCE EM LN882H on Windows natively
...
:: 3. Toolchain
set "CROSS_TOOLCHAIN_ROOT=%CD%\sdk\ZCE_EM7231N\platforms\BK7231N\toolchain\windows\gcc-arm-none-eabi-4_9-2015q1"
if not exist "%CROSS_TOOLCHAIN_ROOT%\bin\arm-none-eabi-gcc.exe" (
    ...
    git submodule update --init --depth=1 sdk/ZCE_EM7231N
)
...
if not exist "sdk\OpenLN882H\project\ZCE EM\app" (
    echo [INFO] Creating junction for app in LN882H ZCE EM project...
    mklink /J "%CD%\sdk\OpenLN882H\project\ZCE EM\app" "%CD%" >nul 2>nul
    if !errorlevel! neq 0 (
...
set BLD_DIR=sdk\OpenLN882H\build-ZCE EM-release\bin
...
if exist "%BLD_DIR%\ZCE EM.bin" (
    copy /y "%BLD_DIR%\ZCE EM.bin" "output\%APP_VERSION%\%OUT_NAME%_OTA.bin" >nul
)
