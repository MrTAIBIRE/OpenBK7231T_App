...
:: ========================================================
:: ZCE EM BK7252N Windows build script
:: Uses Git Bash + bundled ARM GCC toolchain from BK7231N
:: ========================================================

set APP_NAME=ZCE_EM7252N_App
set APP_VERSION=dev_bk7252n
...
echo ==============================================
echo Building ZCE EM for BK7252N on Windows
...
:: 3. Check Windows ARM GCC toolchain in ZCE_EM7231N (borrowed)
if not exist "sdk\ZCE_EM7231N\platforms\BK7231N\toolchain\windows\gcc-arm-none-eabi-4_9-2015q1\bin\arm-none-eabi-gcc.exe" (
    echo [ERROR] Windows ARM GCC toolchain not found!
    echo Needed from: sdk\ZCE_EM7231N\platforms\...
...
"%GIT_BASH%" --login -i "%CD%\build_scripts\build_bk72xx_freertos_inner.sh" "%APP_NAME%" "%APP_VERSION%" "%SDK_DIR%" "%OBK_VARIANT%" "%ACTION%" "bk7252n"
set BUILD_RESULT=%errorlevel%

if %BUILD_RESULT% neq 0 (
    echo.
    echo [ERROR] BK7252N build failed with exit code %BUILD_RESULT%!
    exit /b %BUILD_RESULT%
)
...
if exist "%GEN_DIR%\bk7252n_QIO.bin" (
    copy /y "%GEN_DIR%\bk7252n_QIO.bin" "output\%APP_VERSION%\%APP_NAME%_QIO_%APP_VERSION%.bin" >nul 2>nul
    echo [INFO] Copied: %APP_NAME%_QIO_%APP_VERSION%.bin
    set /a COPIED_COUNT+=1
)
if exist "%GEN_DIR%\bk7252n_UA.bin" (
    copy /y "%GEN_DIR%\bk7252n_UA.bin" "output\%APP_VERSION%\%APP_NAME%_UA_%APP_VERSION%.bin" >nul 2>nul
