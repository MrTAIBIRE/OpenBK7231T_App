...
let commands = [];
let cmdindex = {};	// name -> [cmd docs...], keeps duplicates/variants
let channels = [];
let chanindex = {};
let ios = [];
let ioindex = {};
...
function makeDriverDefineText(defineLines) {
	// defineLines: array of "define" strings - generated by def2text.
...
//console.error(uniq);
	let joined = uniq.map(x=>'"<b>'+x+'</b>"').join(' or ');
	return 'Enabled by defining ' + joined + ' for your platform in [obk_config.h](https://github.com/zce/ZCE_EM_App/blob/main/src/obk_config.h) ';
}

// "extract" C macros matching #define statements 
//
//    even though now there's only "#if" or "#if defined" it handles 
//    I tried to make it more general and handle not only
//    #if, #ifdef, #ifndef, and #if defined(...) conditions
//    but also more complex conditions with && or || or ! 
//
// Actually converts an #if line to an array of condition strings, which can later be converted to a string by 
// the above function "makeDriverDefineText()"
//
// #if A						→ Returns array: ["A"]
// #if defined(B)					→ Returns array: ["B"]
// #if A && B						→ Returns array: ["A && B"]
// #if A || B || (C && !D)				→ Returns array: ["A", "B", "(C && !D)"]
// #ifndef X						→ Returns array: ["!X"]
// #ifdef DEBUG						→ Returns array: ["DEBUG"]
// #if !(A && B) || (C && !D)				→ Returns array: ["!(A && B)", "(C && !D)"]
// #if defined(FEATURE_A) || defined(FEATURE_B)		→ Returns array: ["FEATURE_A", "FEATURE_B"]
function def2text(defline) {
    let line = defline.trim();

    // Handle #ifdef
    if (line.startsWith('#ifdef ')) return [line.substring(7).trim()];
    // Handle #ifndef
    if (line.startsWith('#ifndef ')) return ['!' + line.substring(8).trim()];

    // Handle #if
    if (line.startsWith('#if ')) {
        let condition = line.substring(4).trim();

        // Replace "defined(X)" or "defined X" with "X"
        condition = condition.replace(/defined\s*\(?\s*(\w+)\s*\)?/g, '$1');

        // Split by || while respecting parentheses
        let orParts = splitByOr(condition);

        return orParts.length > 1 ? orParts.map(p => p.trim()) : [condition];
    }
    return [line];
}
// Helper for handling defines containing multiple checks combined with "||"
// they shall be treated like single conditions (will be concatenated later) 
// It splits a string by || while respecting parentheses
				...
				for (let i = 0; i < lines.length; i++) {
					...
					*/
					...
					if (sourceFile && line.startsWith('CMD_RegisterCommand(')) {
						...
						if (cmdindex[cmd.name]) {
							docSameFile = cmdindex[cmd.name].find(x => x.file === cmd.file);
						}
						...
						if (!cmdindex[cmd.name] || cmdindex[cmd.name].length < 1) {
							// it did not have a doc line before
...
function formatDesc(descBasic) {
	if (descBasic != '' && !descBasic.endsWith('.')) {
		descBasic += '.';
	}
...
for (let i = 0; i < commands.length; i++) {
	...
	*/
	...
	let textshort = `| <b>${cmd.name}</b>${cmd.requires ? '<span style="font-size:0.8em;">\nwith <i>' + cmd.requires.replace(' (','</i>\n(')+'</span>' : ''} | ${cmd.args} | ${descBasic}${cmd.examples ? '<br/><br/>Example: ' + cmd.examples : ''}${descMore} |`;
	let textlong = `| <b>${cmd.name}</b>${cmd.requires ? '<span style="font-size:0.8em;">\nwith <i>' + cmd.requires.replace(' (','</i>\n(')+'</span>' : ''} | ${cmd.args}| ${descBasic}${cmd.examples ? '<br/><br/>Example: ' + cmd.examples : ''}${descMore} | File: ${cmd.file}\nFunction: ${cmd.fn} |`;
...
for (let i = 0; i < subpages.length; i++) {
	...
	let base = "https://github.com/zce/ZCE_EM_App/blob/main/";
