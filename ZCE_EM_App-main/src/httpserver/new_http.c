...
const char *g_build_str = "ZCE EM firmware " USER_SW_VER " - " __DATE__ " " __TIME__;
...
int HTTP_ProcessPacket(http_request_t *request)
{
	...
	/* ZCE EM product firmware: expose only the minimal HTTP surface.
	 * Allowed pages: /, /index, /cfg, /cfg_wifi, /cfg_wifi_set, /ota, /ota_exec.
	 * /api/ota is handled by rest_interface.c for local OTA upload.
	 * All other routes from the base project are intentionally not reachable. */
	...
	/* Block disabled ZCE EM/ZCE_EM routes and redirect to product home. */
	return http_fn_index(request);
}
